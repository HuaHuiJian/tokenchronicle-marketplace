...
6. If the local OS scheduler was enabled, rerun `tokenchronicle schedule enable --time HH:MM
   --confirm-background-schedule` from the new runtime. This atomically refreshes the registered command
   so it does not continue invoking the previous Python environment.
